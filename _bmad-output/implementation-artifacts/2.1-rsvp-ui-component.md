...
- [x] Create RSVPSection Component (`src/components/rsvp/RSVPSection.tsx`) (AC: 1)
  ...
  - [x] Display section header: "Подтверждение участия" with gold accents
...
- [x] Create GuestRSVPCard Component (`src/components/rsvp/GuestRSVPCard.tsx`) (AC: 2, 3, 4)
  ...
  - [x] Render two emoji buttons: 👍 (Да) and 👎 (Нет)
...
| **Buttons** | 60px × 60px minimum (larger than 44px requirement) |
...
| **Content** | "✓ Ждём вас!" or "Спасибо за ответ" |
...
│                                                         │
│  ┌───────────────────────────────────────────────────┐  │
│  │  Алексей Петров (ребёнок)       [👍] [👎]        │  │
│  └───────────────────────────────────────────────────┘  │
│                                                         │
│     ┌─────────────────────────────────────────────┐     │
│     │   ✓ Спасибо! Ждём вас на свадьбе!           │     │
│     └─────────────────────────────────────────────┘     │
│              ↑ appears after all responses              │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| **Partial** | Some guests responded | Show progress "2 из 3" |
...
| Touch targets | 60px × 60px buttons (exceeds 44px) |
| Focus states | Visible focus rings, keyboard navigation |
| ARIA | `role="radiogroup"` for Yes/No selection per guest |
| Screen readers | Button labels "Иван Петров: Да" / "Иван Петров: Нет" |
...
```
page.tsx (Server)
    ↓ fetches guests from DB
    ↓ passes as props
RSVPSection (Client)
    ↓ displays all guests
    ↓ manages optimistic state
GuestRSVPCard (Client)
    ↓ calls updateGuestRsvp(guestId, isAttending) on tap
